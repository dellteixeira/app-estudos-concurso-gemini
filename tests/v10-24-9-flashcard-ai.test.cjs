...
test('Workers AI usa parser próprio para flashcard',()=>{assert.match(worker,/function parseFlashcardAIResponse\(result\)/);assert.match(worker,/const parsed = parseFlashcardAIResponse\(result\);/);assert.match(worker,/url\.pathname === "\/api\/ai\/flashcard"/)})
