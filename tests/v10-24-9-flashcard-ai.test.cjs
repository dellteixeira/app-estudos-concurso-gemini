...
test('Workers AI mantém parser próprio como fallback do Gemini',()=>{assert.match(worker,/function parseFlashcardAIResponse\(result\)/);assert.match(worker,/model\?\.provider === "gemini"/);assert.match(worker,/runGeminiFlashcard\(env, model, systemPrompt, userPrompt\)/);assert.match(worker,/parseFlashcardAIResponse\(await withTimeout\(env\.AI\.run\(model\.id/);assert.match(worker,/url\.pathname === "\/api\/ai\/flashcard"/)})
