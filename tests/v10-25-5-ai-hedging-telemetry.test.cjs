...
test('v10.25.6 inicia fallback antecipado sem reduzir o teto do Gemini',()=>{
...
test('v10.25.6 registra sucesso falha e seleção com latência por provedor',()=>{
...
test('v10.25.6 preserva fallback determinístico e versionamento',()=>{
  assert.ok(worker.includes('buildDeterministicFlashcard'));
  assert.ok(worker.includes('fallbackUsed: true, deterministic: true'));
  assert.equal(pkg.version,'10.25.6');
  assert.equal(version.version,'10.25.6');
});
