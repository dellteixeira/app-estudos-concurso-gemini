        ...
        async function createLocalBackupSnapshot(reason = 'alteração automática', options = {}) {
            ...
            const run = async () => {
                ...
                const snapshot = {
                    ...
                    slot:'current',
                    schemaVersion:1,
                    appVersion:String(window.APP_VERSION || document.querySelector('meta[name="app-version"]')?.content || 'desconhecida'),
...
O estado local atual será substituído. Antes da restauração, o Painel preservará automaticamente uma cópia de segurança do estado presente. Depois, as alterações restauradas entrarão na fila normal de sincronização com o Supabase.`,
        ...
        function closeModalByKeyboard(modal) {
            ...
            const closers = {
                ...
                modalPomodoroContext: () => closePomodoroContextModal(),
        ...
        function getSessionMinutes(session) { return StudyDomain.getSessionMinutes(session); }

        function getStudySessionIdentity(session) { return StudyDomain.getStudySessionIdentity(session); }

        function mergeStudySessions(primarySessions, secondarySessions) { return StudyDomain.mergeStudySessions(primarySessions, secondarySessions); }
        ...
        function sortMateriaNamesByCanonicalOrder(names = []) {
            return StudyDomain.sortNamesByCanonicalOrder(names, getCanonicalMateriaOrder());
        }

        function sortAssuntoNamesByCanonicalOrder(materia, names = []) {
            return StudyDomain.sortNamesByCanonicalOrder(names, getCanonicalAssuntosForMateria(materia));
        }
...
` +
        ...
        async function excluirAssuntoEspecifico() {
            ...
            const itemsToDelete = StudyDomain.getTopicItemsForDeletion(editalItems, materia, assunto);
        ...
        function getQuestionProgressFraction(item, contestMeta = null) {
            ...
            return StudyDomain.questionProgressFraction({ total, accuracy, legacyChecked: !!item.questoes });
        }
        ...
        function getTotalRecordedStudyMinutes() {
            // Fonte canônica permanente: studySessions. Pausas nunca entram nessa coleção.
            return StudyDomain.totalStudyMinutes(getStudySessions());
        }
        ...
        let pomodoroContextSelectionPending = false;

        function openPomodoroContextModal() {
            const modal = document.getElementById('modalPomodoroContext');
            const materiaSelect = document.getElementById('pomodoroContextMateria');
            const activitySelect = document.getElementById('pomodoroContextActivity');
            if (!modal || !materiaSelect) return false;
            const materias = getCanonicalMateriaOrder();
            if (!materias.length) {
                void appNotice('Adicione ao menos uma matéria e um assunto no Edital Verticalizado antes de iniciar uma sessão de foco.', { title:'Pomodoro' });
                return false;
            }
            materiaSelect.innerHTML = materias.map(m => `<option value="${escapeHtml(m)}">${escapeHtml(m)}</option>`).join('');
        ...
        function updatePomodoroContextAssuntos() {
            const materia = document.getElementById('pomodoroContextMateria')?.value || '';
            const assuntoSelect = document.getElementById('pomodoroContextAssunto');
            if (!assuntoSelect) return;
            const assuntos = getCanonicalAssuntosForMateria(materia);
            assuntoSelect.innerHTML = assuntos.length
                ? assuntos.map(a => `<option value="${escapeHtml(a)}">${escapeHtml(a)}</option>`).join('')
                : '<option value="">Nenhum assunto disponível</option>';
        }
        ...
        function confirmPomodoroContextAndStart() {
            const materia = document.getElementById('pomodoroContextMateria')?.value || '';
            const assunto = document.getElementById('pomodoroContextAssunto')?.value || '';
            const activityType = document.getElementById('pomodoroContextActivity')?.value || 'teoria';
            if (!materia || !assunto) {
                void appNotice('Selecione uma matéria e um assunto válidos.', { title:'Pomodoro' });
                return;
            }
            const item = editalItems.find(i => i.materia === materia && i.assunto === assunto);
            if (!item) {
                void appNotice('O assunto selecionado não está mais disponível no edital atual.', { title:'Pomodoro' });
                return;
            }
            const todayKey = getLocalDateKey();
            activeStudyContext = {
        ...
        function startTimer() {
            ...
            if (isNewCycle && timerMode === 'focus' && (!activeStudyContext || activeStudyContext.concurso !== currentConcurso)) {
                if (!pomodoroContextSelectionPending) openPomodoroContextModal();
                return;
            }
