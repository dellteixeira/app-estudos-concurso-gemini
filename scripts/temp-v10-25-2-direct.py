...
# Atualiza apenas expectativas de release/arquitetura que mudaram na v10.25.2 para validar a migração.
for test_path in Path('tests').glob('*.test.cjs'):
    text=test_path.read_text()
    text=text.replace('10.25.1','10.25.2')
    if test_path.name in {'library-activation.test.cjs','v10-24-7-library-auto-load.test.cjs'}:
        text=text.replace(r'activationPromise=initialize\(false\)', r'activationPromise=initialize\(true\)')
        text=text.replace('activationPromise=initialize(false)', 'activationPromise=initialize(true)')
    if test_path.name in {'v10-25-1-ai-library-hotfix.test.cjs','v10-25-1-gemini-flashcards.test.cjs'}:
        text=text.replace('gemini-2.5-flash-lite','gemini-2.5-flash')
        text=text.replace('Gemini 2.5 Flash-Lite','Gemini 2.5 Flash')
