...
write('tests/v10-24-7-library-auto-load.test.cjs',`const test=require('node:test');const assert=require('node:assert/strict');const fs=require('node:fs');const r=p=>fs.readFileSync(p,'utf8');test('v10.24.7 biblioteca atualiza ao abrir a guia',()=>{const js=r('public/js/pdf/pdf-library-ui.js'),pkg=require('../package.json');assert.equal(pkg.version,'10.24.7');assert.match(js,/function scheduleLibraryActivationRefresh\(\)/);assert.match(js,/initialize\(true\)\.catch\(handle\)/);assert.match(js,/tab-biblioteca/);assert.match(js,/btn\.dataset\?\.tab==='tab-biblioteca'/);});\n`);
